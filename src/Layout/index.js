...
const Layout = ({
  ...
  const openScrollbar = typeof openScrollbarProps === 'boolean' ? openScrollbarProps : !isMobile() && !window.__COMPONENTS_CORE_SIMPLE_BAR_DISABLED;
  return (<div
    className={classnames(className, 'layout', style['layout'])}
    style={{
      '--menu-max-width': menuMaxWidth, '--menu-min-width': menuMinWidth, '--background': background
    }}
  >
    <Flex>
      <div
        className={classnames('menu', style['menu'], {
          ['open']: menuOpen, [style['open']]: menuOpen
        })}
      >
        ...
        <div className={classnames('menu-inner', style['menu-inner'])}>
          <div className={classnames('menu-header', style['menu-header'])}>
            <ErrorBoundary>{menuHeader ? typeof menuHeader === 'function' ? menuHeader({ menuOpen }) : menuHeader :
              <UserCard menuOpen={menuOpen} {...Object.assign({}, userInfo)} />}</ErrorBoundary>
          </div>
          <ErrorBoundary>
          ...
          {aiDialog && aiType === 'inner' && (
            <Col span={8} className={classnames('page-dialog-outer', style['page-dialog-outer'])}>
              <div className={classnames('page-dialog', style['page-dialog'])}>
                ...
                <ErrorBoundary>
                  <div
                    className={classnames('page-window-content', style['page-window-content'])}>{aiDialog.content}</div>
                </ErrorBoundary>
              </div>
            </Col>)}
          <Col span={aiType === 'inner' ? 16 : 24}>
            <ErrorBoundary>
              {openScrollbar ? <SimpleBar
                className={classnames('page-children', style['page-children'], {
                  ['has-left']: aiType === 'inner'
                })}
              >{children}</SimpleBar> : <div className={classnames('page-children','is-scroller', style['page-children'],style['is-scroller'], {
                ['has-left']: aiType === 'inner'
              })}>{children}</div>}
            </ErrorBoundary>
          </Col>
      ...
      {aiDialog && aiType === 'small' && (<div className={classnames('ai-dialog-window', style['ai-dialog-window'])}>
        <Flex className={classnames('ai-dialog-window-title', style['ai-dialog-window-title'])}>
          <div>{aiDialog.title}</div>
          <Flex gap={10}>
            <Icon
        ...
        <ErrorBoundary>
          <div
            className={classnames('ai-dialog-window-content', style['ai-dialog-window-content'])}>{aiDialog.content}</div>
        </ErrorBoundary>
      </div>)}
    </Flex>
  </div>);
